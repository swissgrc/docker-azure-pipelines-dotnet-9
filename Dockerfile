...
FROM ghcr.io/swissgrc/azure-pipelines-git:2.51.2 AS base
...
LABEL org.opencontainers.image.documentation="https://github.com/swissgrc/docker-azure-pipelines-dotnet-9"
...
RUN apt-get update -y && \
  # Install .NET 9
  apt-get install -y --no-install-recommends dotnet-sdk-9.0=${DOTNET_VERSION}-1 && \
  # Clean up
  apt-get clean && \
  rm -rf /var/lib/apt/lists/* && \
  # Smoke test
